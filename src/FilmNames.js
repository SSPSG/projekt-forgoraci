...
`Kovy`, 
`Zlatá horda`, 
`Fízlové 3: Ve jménu zásad`, 
`Gangs of London`, 
`Sputnik`, 
`Out of Shadows`, 
`Zrádci - Vstup do prázdnoty`, 
`Betaal`, 
...
`Kapitán Říp - Jak získával cenné informace`, 
`Devs`, 
`Nabarvené ptáče`, 
...
`Vyproštění`, 
`Žaluji!`, 
`sKORO NA mizině - Došly prachy`, 
`Bídníci`, 
`Místo zločinu Ostrava`, 
`Papírový dům (Netflix verze) - Série 4`, 
`Luptákův vlogísek`, 
...
`Hra o trůny - Poslední ze Starků`, 
`Krew Boga`, 
`Hellboy`, 
`Klér`, 
`Vlkodlak`, 
...
`Nikdy to nevzdávej 3`, 
`Smrtgasmus`, 
`In fondo al bosco`, 
`Hra peněz`, 
`Hope Lost`, 
`How to Plan an Orgy in a Small Town`, 
`Maigret - Maigret klade past`, 
`Criminal: V hlavě zločince`, 
`The Invitation`, 
`Druhá šance`, 
`Kniha džunglí`, 
`Hardcore Henry`, 
...
`Závod s časem`, 
`Green Street 3: Rváči nikdy nezlomení`, 
`Vittra`, 
...
`Suckablood`, 
`Inspektor Regan`, 
`30 dní paranormální aktivity s ďáblem v těle ženy, která nenávidí muže`, 
`Profesor v ringu`, 
`The Collection`, 
...
`Příšera`, 
`Nerozluční`, 
`Strážce spravedlnosti - Dark Vengeance`, 
`Rock of Ages`, 
`Devil Seed`, 
...
`Muž odnikud`, 
`Sanctum`, 
`Tokugawa onna keibacu emaki: Ušizaki no kei`, 
`Na doraz`, 
`Světová invaze`, 
`Rubber`, 
`Ztracená budoucnost`, 
